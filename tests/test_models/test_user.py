...
class TestUser(unittest.TestCase):
    ...
    def test_places_attr(self):
        """Test User has attr places, and it's an empty list"""
        user = User()
        self.assertTrue(hasattr(user, "places"))
        self.assertEqual(type(user.places), list)
        self.assertEqual(len(user.places), 0)

    @unittest.skipIf(models.storage_t == 'db', "not testing File Storage")
    def test_reviews_attr(self):
        """Test User has attr reviews, and it's an empty list"""
        user = User()
        self.assertTrue(hasattr(user, "reviews"))
        self.assertEqual(type(user.reviews), list)
